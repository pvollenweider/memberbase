...
4. **Matrice rôles (`docs/architecture.html` §15, « Annexe — matrice complète actions/vues/API × rôles »)** : à jour manuellement, pas générée. Toute entrée ajoutée/modifiée/supprimée dans `$ACTION_MAP` (`includes/routing/actions.php`), `$UA_VIEW_ROUTES` (`includes/routing/views.php`), ou tout handler dans `html/api/*.php`, doit se refléter dans le tableau correspondant de cette annexe (fichier, action/vue/endpoint, rôle requis, note). C'est cette annexe qui a mis au jour un vrai trou de sécurité (`DELETE /api/contacts/{id}` sans aucune garde de rôle, corrigé en 5.4.1) — une dérive silencieuse ici rend ce genre d'écart invisible plutôt que de le prévenir.
