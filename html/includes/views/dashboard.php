...
<div class="ca-resume-cards d-flex gap-2 mb-3 flex-wrap">
  ...
  <?php
  // One segmented bar (each type = one colored stretch, width = its share of
  // the total) instead of a per-type bar list: with a dominant category
  // (often >90%), separate bars scaled to the max left the small ones as
  // near-invisible slivers. A single stacked bar shows the actual proportion
  // at a glance; the legend below reuses each segment's color as the key.
  ...
  <div id="dashboardContactTypeBars" style="flex:2 0 0;min-width:260px;background:var(--ca-ground);border:1px solid var(--ca-border,#dee2e6);border-radius:10px;padding:0.85rem 1rem;display:flex;flex-direction:column;gap:0.65rem">
    ...
    <div style="display:flex;height:10px;border-radius:5px;overflow:hidden;background:var(--ca-border,#e2e5e4)">
      ...
      ?>
      <div style="width:<?= round($_ctPct, 2) ?>%;background:<?= $_ctColor ?>;min-width:<?= $_ctPct > 0 ? '2px' : '0' ?>" title="<?= htmlspecialchars($_ctr->label, ENT_QUOTES, $charset) ?>"></div>
      <?php endforeach ?>
    </div>
