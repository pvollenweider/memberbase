...
if ($team != -1) {
    ...
    if ($team == -2) {
        ...
        $query .= "    OR user_properties.parameter='team_19'";
        $query .= " )";
    } else if ($team == -3) {
        $query .= "AND users.id=user_properties.user_id AND NOT (";
...
while ($row = mysql_fetch_object($result)) {
    ...
    if ($team == -4) {
        ...
        # Hide membres in team 12 (soiree 2 oct)
        #if ($user->isMemberOfTeam(12)){
        #    $displayLine = false;
        #}
    }
    ...
    if ($displayLine && ! $user->isMemberOfTeam(19)) {
        ...
        print "$sexe\t$society\t$lastName\t$firstName\t$address\t$npa\t$tel\t$telprof\t$portable\t$fax\t$email\t$web\t$birthday\t$creationDate\t$modificationDate\t$groups\t$coti\n";
    }
}
mysql_free_result($result);
?>
